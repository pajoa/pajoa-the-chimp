...
;(function( win, $, undefined ){
  ...
  ColumnToggle.prototype.init = function() {
    ...
    $menuButton = $( "<a href='#" + id + "' class='btn btn-micro " + this.classes.columnBtn +"' data-popup-link>" +
                    "<span>" + this.i18n.columnBtnText + "</span><i class='rubix-icon fontello icon-fontello-circle'></i></a>" );
    ...
    $(document).bind('click', function(e) {
      ...
      if($popup.find(e.target).length) return;
      if($menuButton.find(e.target).length) return;
      if($popup.is(e.target)) return;
      if($menuButton.is(e.target)) return;
      $popup.hide();
    });
...
;(function( win, $, undefined ){
  ...
  function createSwipeTable( $table ){
    ...
    var $btns = $( "<div class='tablesaw-advance'></div>" ),
      $prevBtn = $( "<a href='#' class='tablesaw-nav-btn btn btn-micro left' title='Previous Column'><i class='rubix-icon fontello icon-fontello-left-open'></i></a>" ).appendTo( $btns ),
      $nextBtn = $( "<a href='#' class='tablesaw-nav-btn btn btn-micro right' title='Next Column'><i class='rubix-icon fontello icon-fontello-right-open'></i></a>" ).appendTo( $btns ),
      hideBtn = 'disabled',
